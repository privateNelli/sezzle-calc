...
2. **frontend-builder** (`node:20-alpine`): `npm ci` and `npm run build` with `VITE_API_BASE_URL=""`.
3. **runtime** (`nginx:1.29-alpine`): copies binary, `dist`, `docker/nginx.conf`, `docker/entrypoint.sh`, installs `dumb-init`.

`dumb-init` is PID 1 so `SIGTERM` reaches both Nginx and the API.

Docker `HEALTHCHECK` requests `http://127.0.0.1/health` (through Nginx).
