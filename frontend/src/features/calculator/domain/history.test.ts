...
describe('history', () => {
  ...
  it('round-trips valid history JSON and ignores corrupt payloads', () => {
    ...
    expect(parseHistory(JSON.stringify({ entries: [] }))).toEqual([])
    expect(parseHistory(JSON.stringify([{ expression: 'broken' }]))).toEqual([])
    expect(parseHistory(JSON.stringify([{ ...entry(), result: Number.POSITIVE_INFINITY }]))).toEqual([])
    expect(parseHistory(JSON.stringify([{ ...entry(), createdAt: 1 }]))).toEqual([])
  })
})
